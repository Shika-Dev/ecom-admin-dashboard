...
Future<PostResponseModel> addProduct({
  ...
  String subCategory='', String imageName='',
  String desc='', String priceSale='', String category='', bool isFeatured=false, String productDetail=''
}) async {
  final request = http.MultipartRequest('POST', Uri.parse('https://api-stg.sevva.co.id/api/product/insert'));
  ...
  request.files.add(http.MultipartFile.fromBytes('image', image, filename: imageName));
...
Future<CategoryModel> getCategory() async {
  final response = await http.get(Uri.https('api-stg.sevva.co.id','api/category/getall'),
      headers: {
...
Future<SubCategoryModel> getSubCategory(String category) async {
  final response = await http.get(Uri.parse('https://api-stg.sevva.co.id/api/subcategory/getallbycategory/$category'));
  if(response.statusCode == 200){
...
Future<PostResponseModel> addCategory({required String category})async{
  ...
  final response = await http.post(Uri.parse(
      'https://api-stg.sevva.co.id/api/category/insert'),
      headers: headers,
...
Future<PostResponseModel> addSubCategory({required String category, required String subcategory})async{
  var headers = {
    'Content-Type': 'application/json'
  };
  Map body = {
  ...
  final response = await http.post(Uri.parse(
      'https://api-stg.sevva.co.id/api/subcategory/insert'),
      headers: headers,
      body: json.encode(body) );
  if(response.statusCode==200){
    return PostResponseModel.fromJson(json.decode(response.body));
...
Future<ProductModel> getProduct() async{
  final response = await http.get(Uri.parse('https://api-stg.sevva.co.id/api/product?limit=500&offset=0'),
      headers: {
        "Access-Control-Allow-Origin": "*",
        'Content-Type': 'application/json',
        'Accept': '*/*'
...
Future<PostResponseModel> updateProduct({
  ...
  final request = http.MultipartRequest('POST', Uri.parse('https://api-stg.sevva.co.id/api/product/update'));
  request.fields['name']=name;
  request.fields['description'] = desc;
  request.fields['category'] = category;
  request.fields['subcategory'] = subCategory;
...
Future<PostResponseModel> deleteProduct({required Product product})async{
  ...
  final response = await http.post(Uri.parse(
      'https://api-stg.sevva.co.id/api/product/update'),
      headers: headers,
      body: json.encode(body) );
  if(response.statusCode==200){
    return PostResponseModel.fromJson(json.decode(response.body));
